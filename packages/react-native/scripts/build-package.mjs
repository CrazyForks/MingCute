...
import { reactNativeAdapter } from '../.tooling/adapter.js';
...
for (const style of iconStyles) {
  const definitions = await mapLimit(await discoverIconSources(sourceRoot, style), 32, async (source) => {
    ...
    await writeFiles(reactNativeAdapter.generateIcon(definition, {
  ...
  await writeFiles(reactNativeAdapter.generateStyleIndex(style, definitions));
  total += definitions.length;
  console.log(`${style}: ${definitions.length}`);
}
await writeFiles(reactNativeAdapter.generateRootIndex(iconStyles));
validateManifest();
console.log(`Generated React Native components: ${total}`);

function validateManifest() {
  const generated = reactNativeAdapter.generatePackageManifest({
    packageName: manifest.name,
  ...
  for (const field of ['exports', 'dependencies', 'peerDependencies', 'sideEffects', 'private']) {
    if (JSON.stringify(generated[field]) !== JSON.stringify(manifest[field])) {
      throw new Error(`React Native package ${field} differs from the adapter-generated manifest.`);
    }
  }
}
