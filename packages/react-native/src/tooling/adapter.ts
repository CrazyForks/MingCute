...
export const reactNativeAdapter: FrameworkAdapter = {
    ...
    return [
      ...
      },
      {
        path: `${basePath}.d.ts`, kind: 'types', contents: [
          ...
          `export declare const ${definition.componentName}: ForwardRefExoticComponent<IconProps & RefAttributes<Svg>>;`,
          `export default ${definition.componentName};`, '',
        ].join('\n'),
      },
    ];
  ...
  generateRootIndex(_styles: readonly IconStyle[]): GeneratedFile[] {
    return [
      ...
      {
        path: 'dist/index.d.ts', kind: 'types', contents: [
  ...
  generatePackageManifest(context: PackageContext): Record<string, unknown> {
    return {
      ...
      peerDependencies: { react: '>=18', 'react-native': '>=0.72', 'react-native-svg': '>=13' },
