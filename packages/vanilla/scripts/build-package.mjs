...
import { vanillaAdapter } from '../.tooling/adapter.js';
...
for (const style of iconStyles) {
  const definitions = await mapLimit(await discoverIconSources(sourceRoot, style), 32, async (source) => {
    ...
    await writeFiles(vanillaAdapter.generateIcon(definition, {
  ...
  await writeFiles(vanillaAdapter.generateStyleIndex(style, definitions));
  total += definitions.length;
  console.log(`${style}: ${definitions.length}`);
}
await writeFiles(vanillaAdapter.generateRootIndex(iconStyles));
validateManifest();
console.log(`Generated Vanilla icon strings: ${total}`);

function validateManifest() {
  const generated = vanillaAdapter.generatePackageManifest({
    packageName: manifest.name,
    version: manifest.version,
    styles: iconStyles,
    registryUrl: manifest.publishConfig.registry,
  });
  for (const field of ['exports', 'dependencies', 'sideEffects', 'private']) {
    if (JSON.stringify(generated[field]) !== JSON.stringify(manifest[field])) {
      throw new Error(`Vanilla package ${field} differs from the adapter-generated manifest.`);
    }
  }
}
