...
const cacheDirectory = await mkdtemp(path.join(os.tmpdir(), 'mingcute-vue-npm-cache-'));
...
try {
  ...
  const sourceFiles = files.filter((filePath) => /^(?:src|scripts|tests|\.tooling)\//.test(filePath));
  ...
  console.log('Vue packed-artifact audit passed.');
} finally {
