...
import { solidAdapter } from '../.tooling/adapter.js';
...
for (const style of iconStyles) {
  const definitions = await mapLimit(await discoverIconSources(sourceRoot, style), 32, async (source) => {
    ...
    await writeFiles(solidAdapter.generateIcon(definition, {
  ...
  await writeFiles(solidAdapter.generateStyleIndex(style, definitions));
  total += definitions.length;
  console.log(`${style}: ${definitions.length}`);
}
await writeFiles(solidAdapter.generateRootIndex(iconStyles));
validateManifest();
console.log(`Generated Solid icon components: ${total}`);

function validateManifest() {
  const generated = solidAdapter.generatePackageManifest({ packageName: manifest.name, version: manifest.version, styles: iconStyles, registryUrl: manifest.publishConfig.registry });
  for (const field of ['exports', 'dependencies', 'peerDependencies', 'sideEffects', 'private']) {
    if (JSON.stringify(generated[field]) !== JSON.stringify(manifest[field])) throw new Error(`Solid package ${field} differs from adapter-generated manifest.`);
  }
}
