...
import { svelteAdapter } from '../.tooling/adapter.js';
...
for (const style of iconStyles) {
  const definitions = await mapLimit(await discoverIconSources(sourceRoot, style), 32, async (source) => {
    ...
    await writeFiles(svelteAdapter.generateIcon(definition, {
  ...
  await writeFiles(svelteAdapter.generateStyleIndex(style, definitions));
  total += definitions.length;
  console.log(`${style}: ${definitions.length}`);
}
await writeFiles(svelteAdapter.generateRootIndex(iconStyles));
validateManifest();
console.log(`Generated Svelte icon components: ${total}`);

function validateManifest() {
  const generated = svelteAdapter.generatePackageManifest({
    packageName: manifest.name,
    version: manifest.version,
    styles: iconStyles,
    registryUrl: manifest.publishConfig.registry,
  });
  for (const field of ['exports', 'dependencies', 'peerDependencies', 'sideEffects', 'private', 'svelte']) {
    if (JSON.stringify(generated[field]) !== JSON.stringify(manifest[field])) {
      throw new Error(`Svelte package ${field} differs from the adapter-generated manifest.`);
    }
  }
}
